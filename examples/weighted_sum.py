...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def forward(ctx, x, weight):
        ...
        print("Tile size: " + repr(ctx.D_TILE_SIZE))
        # Launch our kernel with n instances in our 1D grid.
        n_rows = x.shape[0]  # After rearrange, x is (n_rows, D)

        # Need to initialize empty result tensor as 1D. Note that these elements are not necessarily 0!
        # Important: y must be 1D so stride(0) == 1, otherwise the kernel writes out-of-bounds
    ...
    def backward(ctx, grad_out):
        ...
        # Flatten grad_out so stride(0) == 1, matching what the kernel expects
...
if __name__ == "__main__":
    ...
    print(f"  PASS: {torch.allclose(result, expected)}")
    print()

    # Test 2: 3D input
    print("Test 2: 3D input (batch x seq x dim)")
    ...
    expected = (x_ref * weight_ref).sum(dim=-1)
    expected.sum().backward()

    print(f"  grad_x max diff: {(x.grad - x_ref.grad).abs().max().item()}")
    print(f"  grad_weight max diff: {(weight.grad - weight_ref.grad).abs().max().item()}")
    print(f"  grad_x PASS: {torch.allclose(x.grad, x_ref.grad)}")
    print(f"  grad_weight PASS: {torch.allclose(weight.grad, weight_ref.grad)}")
    print()
